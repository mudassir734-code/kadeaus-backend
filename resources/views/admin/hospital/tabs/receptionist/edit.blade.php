        ...
        <div class="appointment-container">
            ...
            <div class="d-flex align-items-center mb-4">
                <button class="btn btn-link text-dark p-0 me-2">
                    <a href="{{ route('admin.hospital.detail', encrypt($receptionist->hospital_id)) }}#receptionists">&larr;</a>
                </button>
                <h4 class="fw-bold mb-0">Edit Receptionist</h4>
            </div>
            ...
            <form id="receptionist_update_form" action="{{ route('admin.receptionist.update') }}" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ encrypt($receptionist->id) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date"
                               ...
                               name="dob"
                               value="{{ old('dob', $receptionist->user->dob ? date('Y-m-d', strtotime($receptionist->user->dob)) : '') }}"
                               required>
                        @error('dob')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control @error('gender') is-invalid @enderror"
                                name="gender"
                                id="choices-gender-edit"
                                required>
                            <option value="">Select Gender</option>
                            <option value="Male" @selected(old('gender', $receptionist->user->gender) === 'Male')>Male</option>
                            <option value="Female" @selected(old('gender', $receptionist->user->gender) === 'Female')>Female</option>
                            <option value="Other" @selected(old('gender', $receptionist->user->gender) === 'Other')>Other</option>
                        </select>
                        @error('gender')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <select class="form-control @error('role_id') is-invalid @enderror"
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}"
                                        @selected(old('role_id', $receptionist->user->roles->first()?->id) == $role->id)>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control @error('hospital_id') is-invalid @enderror"
                                name="hospital_id"
                                id="choices-hospital-edit"
                                required>
                            <option value="">Select Hospital</option>
                            @foreach($hospitals as $hospital)
                                <option value="{{ $hospital->id }}"
                                        @selected(old('hospital_id', $receptionist->hospital_id) == $hospital->id)>
...
@section('script')
    <script>
        $(document).ready(function() {
            $("#receptionist_update_form").validate({
                rules: {
                    name: {
                        required: true,
                        maxlength: 255
